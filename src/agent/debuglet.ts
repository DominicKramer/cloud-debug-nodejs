...
export class Debuglet extends EventEmitter {
  ...
  constructor(debug: Debug, config: DebugAgentConfig) {
    ...
    /** @private {object} */
    this.config = Debuglet.normalizeConfig(config);
  ...
  static normalizeConfig(config: DebugAgentConfig): DebugAgentConfig {
    const envConfig = {
  ...
  async start(): Promise<void> {
    ...
    SourceMapper.create(mapFiles, async function(err3, sourcemapper) {
      ...
      that.getSourceContext(function(err5, sourceContext) {
        if (err5) {
        ...
        that.scheduleRegistration(0 /* immediately */);
        that.emit('started');
      });
  ...
  static createDebuggee(
    ...
    const uniquifier =
        Debuglet.createUniquifier(desc, version, uid, sourceContext, labels);
   ...
   * @private
   */
  private scheduleRegistration(seconds: number): void {
    const that = this;

    function onError(err: Error) {
      that.logger.error(
          'Failed to re-register debuggee ' + that.project + ': ' + err);
      that.scheduleRegistration(Math.min(
      ...
      that.controller.register(
          ...
          function(err: Error|null, result?: {debuggee: Debuggee;}) {
            ...
            if (!that.fetcherActive) {
              that.scheduleBreakpointFetch(0);
            }
          });
    }, seconds * 1000).unref();
  }
   ...
   * @private
   */
  private scheduleBreakpointFetch(seconds: number): void {
    const that = this;
                ...
                const bps = (body.breakpoints ||
                  ...
                  if (action !== 'CAPTURE' && action !== 'LOG') {
                    ...
                    bp.status = new StatusMessage(
                        ...
                        true);
                    that.rejectBreakpoint(bp);
                    return false;
                  }
                  return true;
                });
                that.updateActiveBreakpoints(bps);
   ...
   * @private
   */
  private updateActiveBreakpoints(breakpoints: stackdriver.Breakpoint[]): void {
    const that = this;
    const updatedBreakpointMap = this.convertBreakpointListToMap(breakpoints);
    ...
    breakpoints.forEach(function(breakpoint: stackdriver.Breakpoint) {
      ...
      if (!that.completedBreakpointMap[breakpoint.id as string] &&
        ...
        // New breakpoint
        that.addBreakpoint(breakpoint, function(err) {
          if (err) {
            that.completeBreakpoint(breakpoint);
          }
        });

        // Schedule the expiry of server breakpoints.
        that.scheduleBreakpointExpiry(breakpoint);
      }
    });
    ...
    Debuglet.mapSubtract(this.activeBreakpointMap, updatedBreakpointMap)
        .forEach(this.removeBreakpoint, this);
  }
   ...
   * @private
   */
  private convertBreakpointListToMap(breakpointList: stackdriver.Breakpoint[]):
   ...
   * @private
   */
  private removeBreakpoint(breakpoint: stackdriver.Breakpoint): void {
   ...
   * @private
   */
  private completeBreakpoint(breakpoint: stackdriver.Breakpoint): void {
    const that = this;
    ...
    that.controller.updateBreakpoint(
        ...
        (that.debuggee as Debuggee), breakpoint, function(err /*, body*/) {
          if (err) {
            ...
            that.removeBreakpoint(breakpoint);
          }
        });
  }
   ...
   * @private
   */
  private rejectBreakpoint(breakpoint: stackdriver.Breakpoint): void {
    const that = this;
   ...
   * @private
   */
  private scheduleBreakpointExpiry(breakpoint: stackdriver.Breakpoint): void {
    const that = this;
  ...
  static format(base: string, exprs: string[]): string {
    const tokens = Debuglet.tokenize(base, exprs.length);
  ...
  private static tokenize(base: string, exprLength: number):
      Array<{v: string}|string> {
    let acc = Debuglet.delimit(base, '$$');
    for (let i = exprLength - 1; i >= 0; i--) {
      ...
      for (let j = 0; j < acc.length; j++) {
        ...
        if ((acc[j] as {v: string}).v) {
          ...
          newAcc.push.apply(
              newAcc, Debuglet.delimit(acc[j] as string, '$' + i));
        }
      }
      acc = newAcc;
    }
  ...
  private static delimit(source: string, delim: string):
      Array<{v: string}|string> {
